...
//        Integer parentRole = parentMenu.getRole();
    ...
    @Operation(summary = "查询所有父级菜单")
    public Result<?> getParentMenus() {
        QueryWrapper<Menu> queryWrapper = new QueryWrapper<>();
        queryWrapper.isNull("pid");
        queryWrapper.isNull("path");
        queryWrapper.isNull("page_path");
        queryWrapper.orderByAsc("sort_num");
    ...
    // 分页查询所有菜单
    @GetMapping("/page")
    @Operation(summary = "分页查询所有菜单")
    public Result<?> page(@RequestParam(defaultValue = "1") Integer currentPage,
                         @RequestParam(defaultValue = "10") Integer size) {
        LOGGER.info("Listing menus, currentPage: {}, size: {}", currentPage, size);
        
        if (currentPage < 1) {
            return Result.error("-1", "页码必须大于0");
        }
        if (size < 1) {
            return Result.error("-1", "每页条数必须大于0"); 
        }
