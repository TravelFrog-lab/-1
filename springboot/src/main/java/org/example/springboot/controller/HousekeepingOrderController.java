...
import org.example.springboot.service.HousekeepingOrderService;
...
public class HousekeepingOrderController {
    ...
    public Result<IPage<HousekeepingOrder>> page(
            ...
            @RequestParam(required = false) Long housekeeperUserId,
            @RequestParam(required = false) String ownerTab,
            @RequestParam(required = false) String housekeeperTab
    ) {
        return housekeepingOrderService.page(pageNum, pageSize, orderNo, status, ownerId, housekeeperId, housekeeperUserId, ownerTab, housekeeperTab);
    }
    ...
    public Result<?> confirm(@PathVariable Long id,
                             @RequestParam(required = false) Long housekeeperUserId) {
        return housekeepingOrderService.confirm(id, housekeeperUserId);
    }

    @PutMapping("/{id}/reject")
    @Operation(summary = "拒接订单")
    public Result<?> reject(@PathVariable Long id,
                            @RequestParam Long housekeeperUserId,
                            @RequestParam String reason) {
        return housekeepingOrderService.reject(id, housekeeperUserId, reason);
    }
    ...
    @Operation(summary = "业主评价已完成家政订单（1-5星+文字，仅一次）")
    public Result<?> submitOwnerEvaluation(
            @PathVariable Long id,
            @RequestParam Long ownerId,
            @RequestBody RepairEvaluationRequest body) {
        return housekeepingOrderService.submitOwnerEvaluation(id, ownerId, body);
    }

    @GetMapping("/checkPayStatus")
    @Operation(summary = "检查订单支付状态")
    public Result<?> checkPayStatus(@RequestParam String orderNo) {
        return housekeepingOrderService.checkPayStatus(orderNo);
    }
} 