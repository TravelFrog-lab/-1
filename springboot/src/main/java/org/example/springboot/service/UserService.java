...
private MaintenanceStaffMapper maintenanceStaffMapper;
    ...
    public Result<?> login(User user){
        try {
            if (user == null || StringUtils.isBlank(user.getUsername()) || user.getPassword() == null) {
            ...
            if (AccountStatus.PENDING_REVIEW.getValue().equals(status)) return Result.error("-1", "账号正在审核");
            if (AccountStatus.REVIEW_FAILED.getValue().equals(status))
                return Result.error("-2", "账号审核不通过，请修改个人信息", compare);
            if (bCryptPasswordEncoder.matches(user.getPassword(), compare.getPassword())) {
                List<Menu> roleMenuList = Collections.emptyList();
                try {
                    List<Menu> all = menuMapper.selectList(null);
                    roleMenuList = all != null ? all : Collections.emptyList();
                } catch (Exception e) {
                    LOGGER.warn("加载菜单失败（请确认已执行完整库脚本并存在 sys_menu 表）: {}", e.getMessage());
                }
            ...
            LOGGER.error("登录异常", e);
            return Result.error("-1", "登录失败，请检查数据库连接或联系管理员");
        }
    }
    ...
    public int createUser(User user) {
        ...
        if (StringUtils.isBlank(user.getStatus())) {
            user.setStatus(AccountStatus.REVIEW_SUCCESS.getValue());
        }
