...
public class PropertyFeeService extends ServiceImpl<PropertyFeeMapper, PropertyFee> {
    ...
    public Result<?> calculatePropertyFee(Long ownerId, String feeDateStr) {
        ...
        // 计算物业费：房屋面积 * 物业费月单价
        BigDecimal amount = house.getArea().multiply(houseType.getPropertyFee());
    ...
    public Result<?> batchCalculatePropertyFee(String feeDateStr) {
        ...
        if (owners.isEmpty()) {
            return Result.error("-1", "没有需要计算物业费的业主");
        }
        ...
        for (Owner owner : owners) {
            ...
            if (result.getCode().equals("0")) {
                ...
                failCount++;
                failMessage.append("业主ID:").append(owner.getId())
                          .append(",失败原因:").append(result.getMsg()).append(";");
            }
        }
        
        String msg = String.format("成功计算%d条记录",
                successCount);
