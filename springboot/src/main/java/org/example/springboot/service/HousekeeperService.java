...
public class HousekeeperService {
    ...
    @Autowired
    private HousekeeperServiceRelationMapper relationMapper;
    ...
    public Result<IPage<Housekeeper>> page(Integer pageNum, Integer pageSize, String name, String phone, String status) {
        ...
        housekeeperPage.getRecords().forEach(housekeeper -> {
            ...
            housekeeper.setRating(housekeepingOrderService.computeAverageRatingForHousekeeper(housekeeper.getId()));
        });
    ...
    public Result<?> save(Housekeeper housekeeper) {
        ...
        // 评分由订单评价均分计算，新增时不写默认分
        housekeeper.setRating(null);
   ...
   Integer deleteRelateService(Long keeperId){
     return  relationMapper.delete(new LambdaQueryWrapper<HousekeeperServiceRelation>().eq(HousekeeperServiceRelation::getHousekeeperId,keeperId) );
    }
