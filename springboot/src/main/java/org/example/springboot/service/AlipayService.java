...
public class AlipayService {
    ...
    public String[] createPaymentOrder(Long userId, BigDecimal amount, String subject,
        ...
        //    支付宝沙盒在本地网络/密钥失效时可能失败，前端此时会拿不到支付链接并认为“无法缴费”。
        //    因为这是演示系统，这里先捕获异常并返回空 payUrl，让上层走“兜底成功”逻辑。
        String payUrl = "";
        try {
            payUrl = createAlipayOrder(paymentOrderNo, amount, subject, alipayConfig.getReturnUrl());
        } catch (Exception e) {
            log.warn("支付宝预下单失败，走兜底逻辑: orderNo={}, amount={}, businessType={}, err={}",
                    paymentOrderNo, amount, businessType, e.toString());
            payUrl = "";
        }
