...
public class HousekeepingOrderService {
     ...
     * 分页查询订单
     */
    public Result<IPage<HousekeepingOrder>> page(Integer pageNum, Integer pageSize, String orderNo, String status, Long ownerId, Long housekeeperId,Long housekeeperUserId) {
        ...
        if (housekeeperUserId != null) {
            List<Housekeeper> housekeepers = housekeeperMapper.selectList(new LambdaQueryWrapper<Housekeeper>().eq(Housekeeper::getUserId, housekeeperUserId));

            LOGGER.info("housekeepers,{}",housekeepers);
