...
public class MaintenanceStaffService extends ServiceImpl<MaintenanceStaffMapper, MaintenanceStaff> {
    ...
    public Page<MaintenanceStaff> list(Integer pageNum, Integer pageSize) {
        ...
        fillAvgEvaluationRating(page.getRecords());
        return page;
    }

    /**
     * 按已完成且有评分的工单计算每位师傅的平均星级（与看板口径一致：仅 COMPLETED + evaluation_rating 非空）。
     */
    private void fillAvgEvaluationRating(List<MaintenanceStaff> staffList) {
        if (staffList == null || staffList.isEmpty()) {
            return;
        }
        List<RepairRecord> rated = repairRecordMapper.selectList(
        ...
        Map<Long, List<Integer>> byMaintainer = rated.stream()
                .filter(r -> r.getMaintainerId() != null && r.getEvaluationRating() != null)
                .collect(Collectors.groupingBy(
                        RepairRecord::getMaintainerId,
                        Collectors.mapping(RepairRecord::getEvaluationRating, Collectors.toList())));
        for (MaintenanceStaff s : staffList) {
            List<Integer> ratings = byMaintainer.get(s.getId());
            if (ratings == null || ratings.isEmpty()) {
                s.setAvgEvaluationRating(null);
                continue;
            }
    ...
    public Result<MaintenanceStaff> getById(Long id) {
        ...
        User user = userMapper.selectById(staff.getUserId());
        staff.setUser(user);
        fillAvgEvaluationRating(Collections.singletonList(staff));
