...
export default {
  ...
  data() {
    return {
      ...
      /** 待办「去缴费」带 autoPay=1 时仅自动发起一次 */
      _autoPayTriggered: false
    }
  },
  computed: {
    statusTagType() {
  ...
  methods: {
    initOrderInfo() {
      ...
      if (query.amount != null && query.amount !== '') {
        const a = parseFloat(String(query.amount).replace(/,/g, ''))
        if (isFinite(a)) this.orderInfo.amount = a.toFixed(2)
      }
      if (query.subject) {
        try {
          this.orderInfo.subject = decodeURIComponent(String(query.subject))
        } catch (e) {
          this.orderInfo.subject = String(query.subject)
        }
      }
      if (query.businessType) {
        try {
          this.orderInfo.businessType = decodeURIComponent(String(query.businessType))
        } catch (e) {
          this.orderInfo.businessType = String(query.businessType)
        }
      }
      if (query.businessOrderNo != null && query.businessOrderNo !== '') {
        try {
          this.orderInfo.businessOrderNo = decodeURIComponent(String(query.businessOrderNo))
        } catch (e) {
          this.orderInfo.businessOrderNo = String(query.businessOrderNo)
        }
      }
      if (query.returnUrl) {
        try {
          this.orderInfo.returnUrl = decodeURIComponent(query.returnUrl)
        } catch (e) {
          this.orderInfo.returnUrl = query.returnUrl
        }
      }
    ...
    /** 首页/消息与待办 物业费一键：进入本页后自动请求下单并打开沙箱 */
    maybeAutoPayFromTodo() {
      const q = this.$route.query
      if (q.autoPay !== '1' && q.autoPay !== 'true') return
      const amt = parseFloat(this.orderInfo.amount)
      if (!isFinite(amt) || amt <= 0) return
      if (this._autoPayTriggered) return
      this._autoPayTriggered = true
      const nextQuery = { ...q }
      delete nextQuery.autoPay
          ...
          if (ok) {
            ...
            if (payUrl && typeof payUrl === 'string') {
              ...
              if (isUrl) {
                ...
                this.step = 2
                this.startPolling()
                  // 不要用 location 覆盖当前页面，否则轮询会被中断
                  // 用新标签打开，当前页面保留并可继续轮询支付状态
