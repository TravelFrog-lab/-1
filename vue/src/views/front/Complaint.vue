        ...
        <el-card class="complaint-form" shadow="never">
          ...
          <el-form ref="complaintForm" :model="complaintForm" :rules="rules" label-width="100px">
            <el-form-item label="投诉类型" prop="complaintType">
              <el-select v-model="complaintForm.complaintType" placeholder="请选择投诉类型" style="width: 100%">
                <el-option
                  v-for="o in complaintTypeOptions"
                  :key="o.value"
                  :label="o.label"
                  :value="o.value"
                />
...
export default {
  ...
  data() {
    return {
      ...
      complaintTypeOptions: [
        ...
        { value: 'FACILITY', label: '设施损坏' },
        { value: 'PARKING', label: '停车相关问题' },
  ...
  methods: {
    getComplaintTypeText(code) {
      if (!code) return '其他'
      const o = this.complaintTypeOptions.find((x) => x.value === code)
      return o ? o.label : '其他'
    },
    // 获取投诉记录
    getComplaintRecords() {
        ...
        if (valid) {
          ...
          Request.post('/complaints', data).then(response => {
            if (response.code === '0') {
              ...
              this.complaintForm.complaintType = 'OTHER'
              this.getComplaintRecords()
            } else {
              this.$message({
                message: response.msg,
