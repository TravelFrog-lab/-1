...
export default {
  ...
  methods: {
    ...
    /** 解析物业费单价（兼容字符串/数字，避免接口字段差异） */
    parsePropertyUnitPrice(houseType) {
      if (!houseType) return 0
      const raw = houseType.propertyFee != null ? houseType.propertyFee : houseType.property_fee
    ...
    /**
     * 获取业主和房屋信息。
     * 额外请求 GET /house-types/{id}，与后台「房屋类型管理」同一数据源，确保修改单价后业主端立即显示新价格；
     * 并同步 Vuex 中的 ownerInfo，避免登录时缓存的旧 house.houseType 在其它页面仍为旧单价。
     */
    getHouseInfo() {
      ...
      Request.get('/owner/' + this.ownerInfo.id).then(response => {
        ...
        const htId = house.houseTypeId

        const applyHouseTypeAndSync = (ht) => {
          const merged = ht || {}
          if (owner.house) {
            owner.house.houseType = merged
          }
          this.$store.commit('SET_OWNER_INFO', owner)

          const area = merged.referenceArea != null && merged.referenceArea !== ''
            ? Number(merged.referenceArea)
            : (getHouseTypeReferenceAreaM2ByName(merged.name) != null
              ? getHouseTypeReferenceAreaM2ByName(merged.name)
              : (house.area != null ? Number(house.area) : 0))
          const unitPrice = this.parsePropertyUnitPrice(merged)
          this.houseInfo = {
            address: house.address || '暂无',
            area,
            typeName: merged.name || '暂无信息',
            unitPrice
          }
          const month = area * unitPrice
          this.currentMonthFee = Number.isFinite(month) ? Math.round(month * 100) / 100 : 0
        }

        if (htId) {
          Request.get('/house-types/' + htId).then(htRes => {
            if (htRes.code === '0' && htRes.data) {
              applyHouseTypeAndSync({ ...houseType, ...htRes.data })
            } else {
              applyHouseTypeAndSync(houseType)
            }
          }).catch(() => applyHouseTypeAndSync(houseType))
        } else {
          applyHouseTypeAndSync(houseType)
        }
      })
