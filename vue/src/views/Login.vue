...
export default {
  ...
  methods: {
    ...mapActions(['login']),
    getKeeperDefaultPath() {
      const menus = this.$store.getters.userMenuList || []
      const leafMenus = []
      menus.forEach(item => {
        if (item.path) leafMenus.push(item)
        if (item.children && item.children.length) {
          item.children.forEach(sub => {
            if (sub.path) leafMenus.push(sub)
          })
        }
      })
      const dashMenu = leafMenus.find(m => m.pagePath === 'HousekeeperDashboard')
      const housekeepingMenu = leafMenus.find(m => m.pagePath === 'HousekeepingOrderManager')
      return (dashMenu && dashMenu.path) ||
        (housekeepingMenu && housekeepingMenu.path) ||
        (leafMenus[0] && leafMenus[0].path) ||
              ...
              if (loginResult.success) {
                try {
                  ...
                  this.$nextTick(() => {
                    ...
                    } else if (res.data.role === 'KEEPER') {
                      this.$router.replace(this.getKeeperDefaultPath());
                    } else {
