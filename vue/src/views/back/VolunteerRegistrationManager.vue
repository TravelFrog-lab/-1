    ...
    <el-card class="menu-card" shadow="never">
      ...
      <el-form ref="searchForm" :inline="true" :model="listQuery" class="search-form">
        ...
        <el-form-item label="报名人姓名">
          <el-input v-model="listQuery.volunteerName" placeholder="请输入报名人姓名" size="medium" />
        </el-form-item>
      ...
      <el-table ref="multipleTable" v-loading="listLoading" :data="tableData" tooltip-effect="dark" row-key="id" style="width: 100%" size="medium"
        ...
        <el-table-column prop="volunteer.user.name" label="报名人姓名"></el-table-column>
  ...
  methods: {
    ...
    handleCancel(row) {
      ...
      this.$confirm('确认取消报名该活动?', '提示', {
        ...
        Request.post(`/community-activity/${row.activity.id}/cancel-registration?volunteerId=${row.volunteer.id}`).then(response => {
          if (response.code === '0') {
            this.$message({
              type: 'success',
              message: '取消报名成功!'
