    ...
    <div class="wrapper">
        ...
        <el-card class="menu-card" shadow="never">
            ...
            <el-table ref="multipleTable" 
                ...
                <el-table-column prop="referenceArea" label="房屋面积" width="120">
                    <template slot-scope="scope">
                        {{ formatReferenceArea(scope.row) }}
                    </template>
                </el-table-column>
...
import { formatHouseTypeReferenceArea } from '@/constants/houseTypeReferenceArea'
...
export default {
    ...
    methods: {
        formatReferenceArea(row) {
            if (row && row.referenceArea != null && row.referenceArea !== '') {
                return `${row.referenceArea} ㎡`
            }
            return formatHouseTypeReferenceArea(row ? row.name : null)
        },
        // 多选操作
        ...
        exportTable(type) {
            if (this.tableData.length) {
                const params = {
                    header: ['ID', '类型名称', '物业费单价', '房屋面积(㎡)', '描述'],
                    key: ['id', 'name', 'propertyFee', 'referenceArea', 'description'],
                    data: this.tableData,
                    autoWidth: true,
                    fileName: '房屋类型数据表',
                    bookType: type
