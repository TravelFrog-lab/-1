        ...
        <el-card class="menu-card" shadow="never">
            ...
            <div class="control-btns">
                ...
                <div class="right-btns">
                    <el-button type="success" size="medium" icon="el-icon-s-promotion" @click="openPushDialog" plain>
                        一键推送到业主端
                    </el-button>
            ...
            <el-alert
                class="property-fee-schedule-tip"
                type="info"
                :closable="false"
                show-icon
                title="每月 10 日 09:00 会自动为业主生成「当月」待缴物业费账单。可随时用手动推送补推其它月份。"
            />
                ...
                <el-table-column label="房屋信息" width="200">
                    <template slot-scope="scope">
                        ...
                        <div style="font-size: 12px;color: #999;">
                            ...
                            （{{ displayHouseAreaM2ForHouse(scope.row.house) }}㎡）
                        </div>
                    </template>
                </el-table-column>
            ...
            <el-dialog
                ...
                <el-form :model="pushForm" label-width="100px">
                    <el-form-item label="费用月份" required>
                        <el-date-picker
                            v-model="pushForm.feeDate"
                            type="month"
                            placeholder="选择要推送的账单月份"
                            format="yyyy-MM"
                            value-format="yyyy-MM"
                            style="width: 100%;">
                        </el-date-picker>
                ...
                <div slot="footer" class="dialog-footer">
                    ...
                    <el-button type="success" :loading="pushing" @click="handlePushToOwners">确认推送</el-button>
                </div>
            </el-dialog>
...
import { displayHouseAreaM2ForHouse } from '@/constants/houseTypeReferenceArea'
...
export default {
    ...
    methods: {
        /** 模板中直接使用，避免未挂载导致 is not a function */
        displayHouseAreaM2ForHouse,
        openPushDialog() {
            this.pushForm.feeDate = moment().format('YYYY-MM')
            this.pushVisible = true
        },
        handlePushToOwners() {
            if (!this.pushForm.feeDate) {
                this.$message({ type: 'warning', message: '请选择费用月份' })
                return
            }
            this.$confirm(
                `确定将 ${this.pushForm.feeDate} 的物业费账单推送到所有符合条件业主的「业主端缴费记录」吗？`,
                '确认推送',
                { type: 'warning', confirmButtonText: '确定推送', cancelButtonText: '取消' }
            ).then(() => {
                this.pushing = true
                // 使用 GET，与 calculateBatch 一致；避免 POST 在部分环境下被 /api 静态资源映射拦截导致 405
                Request.get('/property-fees/pushToOwners', {
                    params: { feeDate: this.pushForm.feeDate }
                })
                    .then(response => {
                        if (response.code === '0' && response.data) {
                            const d = response.data
                            let html = `<div style="line-height:1.7;text-align:left;">
                                <p><strong>费用月份：</strong>${d.feeMonth || '-'}</p>
                                <p><strong>参与业主数：</strong>${d.totalOwners != null ? d.totalOwners : '-'}</p>
                                <p><strong>新推送成功：</strong><span style="color:#67c23a">${d.successCount}</span> 条</p>
                                <p><strong>跳过（该月已存在）：</strong>${d.skipCount} 条</p>
                                <p><strong>失败：</strong><span style="color:#f56c6c">${d.failCount}</span> 条</p>`
                            if (d.failSamples && d.failSamples.length) {
                                html += '<p style="margin-top:10px;"><strong>失败示例：</strong></p><ul style="margin:0;padding-left:18px;">'
                                d.failSamples.forEach(s => {
                                    html += `<li style="word-break:break-all;">${this._escapeHtml(String(s))}</li>`
                                })
                                html += '</ul>'
                            }
                            html += '</div>'
                            this.$alert(html, '推送结果', {
                            ...
                            this.$message({ type: 'error', message: response.msg || '推送失败' })
                        }
                    })
                    .catch(() => {})
                    .finally(() => {
