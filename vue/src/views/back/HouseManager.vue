    ...
    <div class="wrapper">
        ...
        <div class="page-header">
            <div class="header-content">
                ...
                <p class="subtitle">House Management</p>
            </div>
        </div>
            ...
            <el-dialog 
                ...
                <el-form :model="dialogForm" :rules="formRules" ref="dialogForm" label-width="100px">
                    <el-form-item label="房屋地址" prop="address">
                        <el-input v-model="dialogForm.address" placeholder="请输入房屋地址"></el-input>
                    </el-form-item>
                    <el-form-item label="房屋类型" prop="houseTypeId">
                        <el-select v-model="dialogForm.houseTypeId" placeholder="请选择房屋类型" style="width: 100%">
                            <el-option
        ...
        return {
            ...
            formRules: {
                address: [
                    ...
                    { min: 1, max: 200, message: '地址长度必须在1到200个字符之间', trigger: 'blur' }
                ],
                houseTypeId: [
    ...
    methods: {
        ...
        handleSave(formName) {
            this.$refs[formName].validate(valid => {
                ...
                const request = this.dialogForm.id 
                    ? Request.put("/houses", this.dialogForm)
                    : Request.post("/houses", this.dialogForm);
        ...
        exportTable(type) {
            if (this.tableData.length) {
                const params = {
                    header: ['ID', '房屋地址', '房屋类型', '面积', '状态', '创建时间'],
                    key: ['id', 'address', 'houseType.name', 'area', 'status', 'createdAt'],
                    data: this.tableData,
                    autoWidth: true,
                    fileName: '房屋信息表',
                    bookType: type
