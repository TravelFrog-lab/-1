  ...
  <div class="wrapper">
    ...
    <el-card class="menu-card" shadow="never">
      <div class="control-btns">
        <div class="left-btns">
          <el-popconfirm title="确认删除所选投票?" @confirm="batchDelete">
            <template #reference>
              <el-button type="danger" size="medium" plain icon="el-icon-delete">删除</el-button>
            </template>
          </el-popconfirm>
      ...
      <el-dialog
        ...
        width="580px"
        :close-on-click-modal="false"
        @opened="onResultDialogOpened"
        @closed="onResultDialogClosed">
        <div v-if="!resultList.length" class="vote-result-empty">暂无结果数据</div>
        <template v-else>
          <div v-if="resultTotalCount === 0" class="vote-result-empty">暂无投票数据</div>
          <div v-show="resultTotalCount > 0" ref="resultPieChart" class="vote-result-pie" />
        </template>
        <div slot="footer">
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      ...
      this.$refs.dialogForm.validate(valid => {
        ...
        const payload = {
          ...this.dialogForm,
          options: opts.map(o => {
            const row = { optionText: o.optionText.trim() }
            if (o.id != null && o.id !== '') row.id = o.id
            return row
          })
        }
    ...
    onResultDialogOpened() {
      this.$nextTick(() => this.renderResultPie())
    },
    onResultDialogClosed() {
      this.disposeResultPie()
    },
    disposeResultPie() {
      if (this._resultPieResizeHandler) {
        window.removeEventListener('resize', this._resultPieResizeHandler)
        this._resultPieResizeHandler = null
      }
      if (this.resultPieChart) {
        this.resultPieChart.dispose()
        this.resultPieChart = null
      }
    },
    renderResultPie() {
      if (!this.resultList.length || this.resultTotalCount <= 0) return
      const el = this.$refs.resultPieChart
      if (!el) return
      this.disposeResultPie()
      ...
      this.resultPieChart = echarts.init(el)
      this.resultPieChart.setOption({
      ...
      this._resultPieResizeHandler = () => {
        if (this.resultPieChart) this.resultPieChart.resize()
      }
      window.addEventListener('resize', this._resultPieResizeHandler)
    },
    exportResult() {
...
.vote-result-pie { width: 100%; height: 380px; min-height: 320px; }
.vote-result-empty {
  padding: 48px 20px;
  text-align: center;
  color: #909399;
