<template>
  <div class="wrapper">
    ...
    <el-card class="menu-card" shadow="never">
      ...
      <!-- 按处理状态分类 -->
      <el-tabs v-model="adminComplaintTab" class="complaint-admin-tabs" @tab-click="onComplaintTabClick">
...
export default {
  ...
  data() {
    return {
      ...
      /** 全部 / 待处理 / 处理中 / 已完成（对应 PENDING / PROCESSING / RESOLVED） */
      adminComplaintTab: 'all',
  ...
  created() {
    if (this.currentUser && this.currentUser.role === 'MAINTENANCE') {
      this.$message.warning('投诉建议由管理员统一处理')
  ...
  methods: {
    ...
    onComplaintTabClick(tab) {
      if (tab && tab.name) {
        this.adminComplaintTab = tab.name
      }
    ...
    fetchData({ page, limit } = {}) {
      ...
      if (this.adminComplaintTab && this.adminComplaintTab !== 'all') {
        params.status = tabStatusMap[this.adminComplaintTab]
      }
